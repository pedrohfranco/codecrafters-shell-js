...
function inPATH(cmd, returnPath = false) {
  ...
  for(let i = 0; i < PathLength; i++) {
    ...
    if(isSep) {
      ...
      if(pathExists(currentPath, cmd)) {
        if(returnPath) {
          return pathThatExists(currentPath, cmd);
        }
...
function pathThatExists(path, cmd) {
  if(!path.endsWith("/")) {
    path = path.concat("/");
  }
